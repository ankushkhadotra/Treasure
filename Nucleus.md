...
\begin{tikzpicture}[
    ...
    % Define block styles
    compartment/.style = {rectangle, draw=black, fill=gray!10, rounded corners, minimum width=4cm, minimum height=0.8cm, font=\bfseries},
    filament/.style = {rectangle, draw=black, fill=blue!5, minimum width=4.5cm, minimum height=0.6cm},
    ring/.style = {ellipse, draw=black, fill=orange!10, minimum width=5cm, minimum height=0.7cm, align=center},
    ...
    \node (ring1)  [ring, below=of fil1] {Cytoplasmic Ring};
    \node (ring2)  [ring, below=of ring1] {Luminal / Spoke Ring \\ \small(Anchors to INM/ONM Fusion)};
    \node (ring3)  [ring, below=of ring2] {Inner Scaffold Ring \\ \small(Lines the Central FG-Nup Pore)};
    \node (ring4)  [ring, below=of ring3] {Nuclear Ring};
...
\end{tikzpicture}
\caption{Structural Hierarchy of the Nuclear Pore Complex (NPC)}
\label{fig:npc_structure}
\end{figure}
...
- **Passive Diffusion:** Small molecules, ions, and proteins (
    
    ![](data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==)<40
    
    ![](data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==)−50
    
    ![](data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==) kDa
    
    ) diffuse freely through the aqueous channels of the NPC. 
    
- **Active Mediated Transport:** Macromolecules (
    ...
    , such as RNA polymerases, histones, and fully formed ribosomal subunits) require specialized transport receptors (**Karyopherins**) and energy. 
...
2. **Receptor Binding:** The NLS is recognized and bound by a soluble cytoplasmic receptor called **Importin** (
    
    ![](data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==)𝛼
...
- **Nucleoplasm (Karyoplasm):** The clear, highly viscous protoplasmic fluid core containing high concentrations of nucleotides, cofactors, enzymes (DNA and RNA polymerases), and inorganic ions (
    ...
    Mn2+Mn raised to the 2 plus power
